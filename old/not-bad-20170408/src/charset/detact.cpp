...
static double ___chinese_score(const char *fromcode, char *str, int len, int omit_invalid_bytes_count)
{
    ...
    zcc_inner_debug("        # %-20s, score:%ld, count:%d, omit:%d"
            , fromcode, score, count, omit_invalid_bytes_count);
...
bool charset_detect(const char *data, size_t len, char *charset_result, const char **charset_list)
{
    ...
    for (i = 0; i < list_len; i++) {
        ...
        if (ret < 0) {
            zcc_inner_debug("        # %-20s, iconv failure", fromcode);
            continue;
        }
        if (omit_invalid_bytes_count > 5) {
            zcc_inner_debug("        # %-20s, omit_invalid_bytes: %d", fromcode, omit_invalid_bytes_count);
            continue;
        }
        if (converted_len < 1) {
            continue;
