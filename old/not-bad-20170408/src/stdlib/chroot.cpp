...
bool chroot_user(const char *root_dir, const char *user_name)
{
    ...
    if (user_name != 0) {
        if ((pwd = getpwnam(user_name)) == 0) {
            log_fatal("chroot_user: unknown user: %s", user_name);
            errno = ENONET;
            return false;
        }
        uid = pwd->pw_uid;
        gid = pwd->pw_gid;
        if (setgid(gid) < 0) {
            log_fatal("chroot_user: setgid %ld : %m", (long)gid);
            return false;
        }
        if (initgroups(user_name, gid) < 0) {
            log_fatal("chroot_user: initgroups: %m");
            return false;
        }
    }
    ...
    if (root_dir) {
        if (chroot(root_dir)) {
            log_fatal("chroot_user: chroot (%s) : %m", root_dir);
            return false;
        }
        if (chdir("/")) {
            log_fatal("chroot_user: chdir (/): %m");
            return false;
        }
    }
    ...
    if (user_name != 0) {
        if (setuid(uid) < 0) {
            log_fatal("chroot_user: setuid %ld: %m", (long)uid);
            return false;
        }
    }
