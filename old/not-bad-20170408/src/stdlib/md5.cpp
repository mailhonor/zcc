 ...
 */
static void *___body_20161203(struct md5_runtime * ctx, const void *data, size_t size)
{
    unsigned char *ptr;
...
md5 *md5::update(const void *data, size_t size)
{
    ...
    if (used) {
        ...
        size -= free;
        ___body_20161203 (ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = ___body_20161203 (ctx, data, size & ~(unsigned long) 0x3f);
        size &= 0x3f;
    }
...
char *md5::get_result(void *result)
{
    struct md5_runtime *ctx = &(_ctx);
    /* @UNSAFE */
    unsigned long used, free;
