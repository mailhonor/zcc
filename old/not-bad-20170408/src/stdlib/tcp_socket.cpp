...
static int ___sane_connect(int sock, struct sockaddr *sa, socklen_t len)
{
    ...
    return (___ZCC_connect(sock, sa, len));
}
...
static int ___timed_connect(int sock, struct sockaddr *sa, int len, long timeout)
{
    ...
    if (timeout > 0) {
        ...
        if (errno != EINPROGRESS)
            return (-1);
        if (wait_writeable(sock, timeout) < 0)
            return (-1);
...
int host_connect(const char *host, int port, long timeout)
{
    ...
    for (i = 0; i < count; i++) {
        sock = inet_connect(ip_list + i*16, port, timeout);
        if (sock < 1) {
            return -1;
        }
        return sock;
...
int connect(const char *netpath, long timeout)
{
    ...
    if (p) {
        *p = 0;
        port = atoi(p + 1);
        fd = host_connect(_netpath, port, timeout);
    } else {
        fd = unix_connect(_netpath, timeout);
    }
