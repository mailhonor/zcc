...
static int (*___ZCC_open)(const char *, int, ...) = open;
...
bool file_put_contents(const char *filename, const void *data, size_t len)
{
    ...
    while ((fd = ___ZCC_open(filename, O_CREAT | O_RDWR | O_TRUNC, 0777)) == -1 && errno == EINTR) {
        continue;
    }
...
ssize_t file_get_contents(const char *filename, string &str)
{
    ...
    while ((fd = ___ZCC_open(filename, O_RDONLY)) == -1 && errno == EINTR) {
        continue;
    }
    if (fd == -1) {
        return -1;
...
ssize_t file_get_contents_sample(const char *filename, string &str)
{
    ...
    if (ret < 0) {
        log_info("ERR: load from %s (%m)", filename);
        exit(1);
    }
    return ret;
}
