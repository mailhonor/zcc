...
#define zdebug(fmt, args...) if (openssl_debug_enable) {zcc_inner_debug(fmt,##args);}

bool openssl_debug_enable = false;
...
#define ___Z_SSL_TIMED_DO(excute_sentence)  \
    ...
    for (;;) { \
        ...
        case SSL_ERROR_WANT_WRITE: \
            ...
            if (!wait_writeable(_fd, left_time)) { ret=-1; goto over; }\
            break; \
        case SSL_ERROR_WANT_READ: \
            if ((left_time = timeout_left(cirtical_time)) < 1) { ret = -1; goto over;} \
            if (!wait_readable(_fd, left_time)) { ret = -1; goto over; } \
            break; \
        case SSL_ERROR_ZERO_RETURN:  /* FIXME */ \
        case SSL_ERROR_SSL: \
        case SSL_ERROR_SYSCALL: \
        default: \
             zdebug("openssl: found error"); \
             ret = -1; goto over; \
             break; \
        } \
    } \
