...
{
    printf("USAGE: %s -f eml_filename [ eml_fn2 ... ] [ -mpool ] [-att ] \n", zcc::var_progname);
    exit(1);
}
...
static void do_parse(char *eml_fn)
{
    ...
    zcc::mem_pool *mp = &zcc::system_mem_pool_instance;
    zcc::greedy_mem_pool greedy_mpool;
    if (enable_mpool) {
        mp = &greedy_mpool;
    }

    zcc::mail_parser parser(*mp);
...
int main(int argc, char **argv)
{
    ...
    zcc_vector_walk_begin(fn_vec, eml_fn) {
        do_parse(eml_fn);
    } zcc_vector_walk_end;;
