...
#define lock_evbase(eb) {if((eb)->lock){(eb)->lock->lock();}}
#define unlock_evbase(eb) {if((eb)->lock){(eb)->lock->unlock();}}
...
static inline int aio_timer_check(evbase_t * eb)
{
    ...
    while (1) {
        ...
        if (callback) {
            ...
            zcc_fatal("aio: not found callback");
        }
    }
        ...
        if (events == 0) {
            if (old_events) {
                if (epoll_ctl(eb->epoll_fd, EPOLL_CTL_DEL, fd, NULL) == -1) {
                    zcc_fatal("aio_event_set: fd %d: DEL  ssl_error: %m", fd);
                }
            }
            ...
            if (epoll_ctl(eb->epoll_fd, (old_events ? EPOLL_CTL_MOD : EPOLL_CTL_ADD), fd, &evt) == -1) {
                zcc_fatal("aio_event_set: fd %d: %s ssl_error: %m", fd, (old_events ? "MOD" : "ADD"));
            }
        }
    }
...
event_base::~event_base()
{
    ...
    if (eb->lock && eb->lock_auto_release) {
        delete eb->lock;
    }
}
...
void event_base::dispatch(long default_delay)
{
    ...
    if (nfds == -1) {
        if (errno != EINTR) {
            zcc_fatal("zbase_dispatch: epoll_wait: %m");
        }
        return;
    }
...
void event_base::set_locker(locker *lock, bool auto_release)
{
    ___data.lock = lock;
    ___data.lock_auto_release = auto_release;
}

locker *event_base::get_locker()
{
    return ___data.lock;
}
...
{
    aio_t *lh = (aio_t *)(*list_head), *lt = (aio_t *)(*list_tail), *a = &(aio->___data);
    zcc_mlink_append(lh, lt, a, queue_prev, queue_next);
...
{
    aio_t *lh = (aio_t *)(*list_head), *lt = (aio_t *)(*list_tail), *a = &(aio->___data);
    zcc_mlink_detach(lh, lt, a, queue_prev, queue_next);
    *list_head = (async_io *)lh;
    *list_tail = (async_io *)lt;
}
