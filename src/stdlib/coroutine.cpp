...
public:
    ...
    list<coroutine_mutex_t *> *___mutex_list;
    /* flags */
...
static void coroutine_append_mutex(coroutine *co, coroutine_mutex_t *mutex)
{
    if (co->___mutex_list == 0) {
        co->___mutex_list = new list<coroutine_mutex_t *>();
        co->___mutex_list->push_back(mutex);
        return;
    }

    zcc_list_walk_begin(*(co->___mutex_list), m) {
        if (m == mutex) {
            return;
        }
    } zcc_list_walk_end;
...
{
    list<coroutine_mutex_t *>::node *del = 0;
    zcc_list_walk_begin(*(co->___mutex_list), m) {
        if (m == mutex) {
            del = var_zcc_list_node;
        }
    } zcc_list_walk_end;
    if (del) {
        co->___mutex_list->erase(del);
    }
    if (co->___mutex_list->size() == 0) {
...
static void coroutine_release_all_mutex(coroutine *co)
{
    ...
    size_t i, count = co->___mutex_list->size();
    for (i = 0; i <count; i++) {
        list<coroutine_mutex_t *>::node *node = co->___mutex_list->last_node();
        if (node==0) {
            break;
        }
        coroutine_mutex_t *mutex = (coroutine_mutex_t *)(node->data());
...
void coroutine_mutex_lock(coroutine_mutex_t *m)
{
    ...
    list<coroutine *> &colist = m->colist;
    list_node *node = colist.first_node();
    if ((!node)) {
        colist.push_back(co);
        coroutine_append_mutex(co, m);
        return;
    }
    if ((node->data() == (void *)co)) {
        coroutine_append_mutex(co, m);
        return;
    }
    colist.push_back(co);
...
void coroutine_mutex_unlock(coroutine_mutex_t *m)
{
    ...
    if ((coroutine *)node->data() != co) {
        return;
    }
    coroutine_remove_mutex(co, m);
    colist.shift(0);
    node = colist.first_node();
    if (node == 0) {
        return;
    }
    co = (coroutine *)node->data();
